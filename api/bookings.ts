...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  //    it) directly in Outlook, the calendar is the truth. The shared
  //    reconciler moves or clears the database row to match (freeing the old
  //    window on the site) and sends the branded reschedule emails to the
  //    owner and the customer when it detects a drag.
  try {
    const sql = neon(process.env.DATABASE_URL!);
    const host = (req.headers["x-forwarded-host"] as string) || (req.headers.host as string) || "www.belvederedecks.com";
    const { active } = await reconcileRange(sql, `https://${host}`, startKey, endKey);
    for (const r of active) add(r.dateKey, r.windowId);
  } catch {
